...
class SimpleConv(pl.LightningModule):
    def __init__(self, learning_rate, mode: ["classification", "regression"] = "classification"):
        super().__init__()
        self.mode = mode
        ...
        #self.fc22 = nn.Linear(10, 2)  # for classification
    ...
    def training_step(self, batch, batch_idx):
        ...
        if self.mode == "classification":
            ...
            self.log("classification_loss", class_loss, on_step=True, on_epoch=False)
    ...
    def training_epoch_end(self, outs):
        ...
        if self.mode == "classification":
            ...
            #mlflow.log_metric('accuracy', float(self.train_acc.compute()))
            #mlflow.log_metric('step', self.trainer.current_epoch)
        else:
            pass
    ...
    def validation_step(self, batch, batch_idx):
        ...
        if self.mode == "classification":
            ...
            self.log("valid_acc", self.valid_acc.compute(), on_step=True, on_epoch=False)
            self.log("classification_loss", class_loss, on_step=True, on_epoch=False)
            return class_loss
    ...
    def validation_epoch_end(self, outs):
        ...
        if self.mode == "classification":
            # log epoch metric
            self.log('step', self.trainer.current_epoch, on_step=False, on_epoch=True)
            self.log("valid_acc_epoch", self.valid_acc.compute(), on_step=False, on_epoch=True)
        else:
            pass
